...
ProperyBucket is a lightweight library designed to provide a type-safe and flexible way to manage dynamic properties. Unlike traditional static objects where properties must be defined at compile time, ProperyBucket allows you to add, remove, and manage properties during runtime, making it perfect for scenarios where the structure of your data isn't known until the application is running.
...
```bash
# Using npm
npm install propery-bucket

# Using yarn
yarn add propery-bucket

# Using pnpm
pnpm add propery-bucket
```

## Quick Start

```javascript
import { ProperyBucket } from 'propery-bucket';

// Create a new ProperyBucket instance
const bucket = new ProperyBucket();
...
```javascript
new ProperyBucket(options?: BucketOptions)
```

Creates a new ProperyBucket instance with optional configuration.
...
#### `set(key: string, value: any): ProperyBucket`
...
```javascript
const bucket = new ProperyBucket({ strict: true });
...
```javascript
const bucket = new ProperyBucket({ immutable: true });
...
class UserProfile {
  constructor(userId) {
    this.userId = userId;
    this.properties = new ProperyBucket();
  }
...
class PluginManager {
  ...
  registerPlugin(pluginName, plugin) {
    const pluginBucket = new ProperyBucket();
...
- ProperyBucket uses efficient internal storage mechanisms
...
ProperyBucket works in both browser and Node.js environments:
...
git clone https://github.com/tazayan/ProperyBucket.git
cd ProperyBucket
...
- **Documentation**: [https://github.com/tazayan/ProperyBucket/wiki](https://github.com/tazayan/ProperyBucket/wiki)
- **Issue Tracker**: [https://github.com/tazayan/ProperyBucket/issues](https://github.com/tazayan/ProperyBucket/issues)
- **Discussions**: [https://github.com/tazayan/ProperyBucket/discussions](https://github.com/tazayan/ProperyBucket/discussions)
...
Special thanks to all contributors who have helped make ProperyBucket better!

---

Made with ❤️ by the ProperyBucket team